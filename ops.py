...
def ssim(img1, img2, sd=1.5 , c1=(0.01*7)**2, c2=(0.03*7)**2):
  mul1 = gaussian_filter(img1, sd)
  mul2 = gaussian_filter(img2, sd)
  mul1_mul2 = mul1*mul2
  mul1_sq = mul1*mul1
  mul2_sq = mul2*mul2
  sigma1_sq = gaussian_filter(img1*img1, sd)-mul1_sq
  sigma2_sq = gaussian_filter(img2*img2, sd)-mul2_sq  

  sigma12 = gaussian_filter(img1*img2,sd)-mul1_mul2
  ssim_num = ((2*mul1_mul2+c1)*(2*sigma12+c2))
  ssim_den = ((mul1_sq+mul2_sq+c1)*(sigma1_sq+sigma2_sq+c2)) 
...
def psnr(imageA, imageB):
  mse_err = mse(imageA, imageB)
  if mse_err ==0:
    return 100
  else:
    return 10 * math.log10(255.0/math.sqrt(mse_err))
